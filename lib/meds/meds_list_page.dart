...
class MedicationForm extends StatefulWidget {
  ...
  MedicationForm.empty()
      : id = '', // No ID for new medication
        name = 'New Medication',
        dose = '0.5',
        type = 'Pill',
        measurement = 'mg',
        startDate = DateTime.now(), // Default to current date
        endDate = DateTime.now().add(Duration(days: 14)), // Default to 30 days from now
        ...
        body: Padding(
          ...
          child: LayoutBuilder(
              builder: (context, constraints) {
                // Calculate the width for each ChoiceChip
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: medicationTypes.map((medication) {
                        return SizedBox(
                          width: 56, // Set a larger fixed width for all chips
                          child: ChoiceChip(
                            showCheckmark: false,
                            label: Column(
                              ...
                              children: [
                                Icon(medication.icon, size: 24), // Display the icon
                                SizedBox(height: 4), // Small space between icon and text
                              ],
                            ),
                            selected: _medicationType == medication.name, // Track selection based on name
                            selectedColor: const Color(0xFF8BACA5),
                            onSelected: (selected) {
                              setState(() {
                                _medicationType = medication.name; // Update the selected medication type
                              });
                            },
                          ),
                        );
                      ...
                      children: mealTimes.map((time) {
                        return SizedBox(
                          ...
                          child: ChoiceChip(
                            ...
                            showCheckmark: false,
                            selectedColor: const Color(0xFF8BACA5),
                            label: Center( // Center the text inside the chip
                              child: Text(
                                time,
                                style: TextStyle(fontSize: 12), // Adjust the font size if needed
                              ),
                            ),
                    ...
                    TextButton(
                      onPressed: () {
                        setState(() {
                          _schedules.add(
                              '12:00'); // Default time for a new dose
                        });
                      },
                      child: Text('+ Add Dose'),
                    ),
                    ...
                    ListTile(
                      ...
                      onTap: () {
                        // Add reminder offset picker here
                        showDialog(
                          context: context,
                    ...
                    Wrap(
                      ...
                      children: days.map((day) {
                        ...
                        return SizedBox(
                          width: daysChipWidth, // Set a fixed width for all chips
                          child: ChoiceChip(
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: () {
                            // Save medication with all the selected details
