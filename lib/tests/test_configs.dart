...
final Map<String, TestConfig> testConfigurations = {
  ...
  'gad7': TestConfig(
    ...
    testName: 'gad7',
    category: 'Тревожность',
    description: 'Тест GAD-7 помогает определить уровень тревожности и оценить, насколько сильно беспокойство влияет на повседневную жизнь.',
    calculateResults: (answers) {
      int totalScore = 0;
      ...
      if (totalScore <= 4) {
        return {
          'Итог': totalScore,
          'Результат':  'Низкий уровень тревожности. Возможно, состояние не требует лечения, но стоит следить за его изменениями.',
        };
      } else if (totalScore <= 9) {
        return {
          'Итог': totalScore,
          'Результат':   'Умеренный уровень тревожности. Рекомендуется консультация с психологом или психотерапевтом.',
        };
      } else if (totalScore <= 14) {
        return {
          'Итог': totalScore,
          'Результат':   'Выраженный уровень тревожности. Рекомендуется психотерапия или медикаментозное лечение.',
        };
      } else {
        return {
          'Итог': totalScore,
          'Результат':   'Серьезная тревожность. Необходима срочная помощь специалиста.',
        };
      }
    },
  ),
  'pss': TestConfig(
    name: 'Шкала воспринимаемого стресса',
    testName: 'pss',
    category: 'Тревожность',
    description: 'Этот тест оценивает уровень стресса, который вы испытываете в своей жизни, и помогает понять, насколько стресс влияет на ваше благополучие.',
    calculateResults: (answers) {
      int totalScore = 0;
      ...
      if (totalScore <= 5) {
        return {
          'Итог': totalScore,
          'Результат': 'Низкий уровень стресса. Вы хорошо справляетесь с жизненными трудностями.',
        };
      } else if (totalScore <= 15) {
        return {
          'Итог': totalScore,
          'Результат': 'Умеренный уровень стресса. Возможно, вам будет полезно научиться лучше управлять стрессом.',
        };
      } else if (totalScore <= 25) {
        return {
          'Итог': totalScore,
          'Результат': 'Высокий уровень стресса. Рекомендуется обратить внимание на методы расслабления или обратиться за помощью.',
        };
      } else {
        return {
          'Итог': totalScore,
          'Результат': 'Очень высокий уровень стресса. Возможно, вам нужно немедленно обратиться за помощью к специалисту.',
        };
      }
    },
  ),
